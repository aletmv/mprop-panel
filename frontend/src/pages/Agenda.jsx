...
import { proximasFirmas } from '@/lib/mockData';

const dias = ['Lun', 'Mar', 'Mié', 'Jue', 'Vie', 'Sáb', 'Dom'];

// Mock month: Junio 2025 (starts on Sunday → for simplicity, render 5 weeks)
const buildMonth = () => {
  // Build a 5x7 grid for june 2025: 1 jun = sunday → shift to 7
  const cells = [];
  // Previous month tail
  ...
  while (cells.length < 42) {
    cells.push({ day: cells.length - i + 1 - (30 - 0), next: true });
  }
  ...
  return (
    <>
      <Topbar
        ...
        subtitle="Planificá y reprogramá las firmas de tu escribanía"
      />
      ...
      <div className="p-6 lg:p-8 max-w-[1500px] mx-auto space-y-5">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div className="inline-flex items-center bg-card border border-border rounded-lg h-10">
            <Button variant="ghost" size="icon" className="h-10 w-10 rounded-r-none"><ChevronLeft className="w-4 h-4" /></Button>
            <div className="px-4 text-[14px] font-semibold text-foreground border-x border-border h-10 leading-10">{mes}</div>
            <Button variant="ghost" size="icon" className="h-10 w-10 rounded-l-none"><ChevronRight className="w-4 h-4" /></Button>
          </div>
        ...
        <div className="grid grid-cols-12 gap-5">
          ...
          <div className="col-span-12 lg:col-span-8 card-surface overflow-hidden">
            <div className="grid grid-cols-7 border-b border-border">
              {dias.map(d => (
                <div key={d} className="px-3 py-3 text-[11px] uppercase tracking-wider font-semibold text-muted-foreground text-center">{d}</div>
              ))}
            </div>
            <div className="grid grid-cols-7">
              {cells.map((c, i) => {
                ...
                return (
                  ...
                  >
                    <div className={`inline-flex w-7 h-7 items-center justify-center rounded-full text-[12px] font-semibold ${
                    ...
                    {e && !c.prev && !c.next && (
                      <div className="mt-1 space-y-1">
                        {e.map(ev => {
                          ...
                          return (
                            <Link
                              to={`/operaciones/${ev.id}`}
                              key={ev.id}
                              className={`block px-1.5 py-1 rounded-md border ${cls} text-[10px] font-mono truncate hover:shadow-soft transition-shadow`}
                            >
            ...
            <div className="card-surface p-5">
              ...
              <div className="space-y-3">
                {proximasFirmas.map(f => {
                  ...
                  return (
                    <Link to={`/operaciones/${f.operacion}`} key={f.operacion} className="block p-3 rounded-xl border border-border hover:border-primary/30 hover:shadow-soft transition-all bg-muted/20">
            ...
            <div className="rounded-2xl p-4 bg-accent-soft border border-accent/20">
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-[12px] text-muted-foreground mt-1">Conectá tu calendario para sincronizar firmas y recibir recordatorios automáticos.</p>
