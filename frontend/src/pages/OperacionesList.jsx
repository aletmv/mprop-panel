...
import {
  ...
  CheckCircle2, AlertCircle, ChevronRight, Download
} from 'lucide-react';
import { operaciones, estadoLabel, riesgoLabel } from '@/lib/mockData';
...
const OperacionesList = () => {
  ...
  const lista = useMemo(() => {
    return operaciones.filter(o => {
      ...
      const matchQ = !q || o.direccion.toLowerCase().includes(q) || o.id.toLowerCase().includes(q) || o.vendedor.nombre.toLowerCase().includes(q) || o.comprador.nombre.toLowerCase().includes(q);
  ...
  return (
    <>
      <Topbar
        title="Operaciones"
        subtitle="Todos los legajos en gestión por tu escribanía"
      />
      ...
      <div className="p-6 lg:p-8 max-w-[1500px] mx-auto space-y-5">
        ...
        <div className="flex flex-wrap gap-2">
          {filtros.map(f => (
            ...
            >
              {f.label}
              <span className={`ml-1.5 text-[11px] font-bold ${filtro === f.id ? 'opacity-80' : 'opacity-50'}`}>{f.count}</span>
            </button>
          ))}
        </div>
        ...
        <div className="card-surface overflow-hidden">
            ...
            <table className="w-full text-[13px]">
                ...
                <tr className="bg-muted/50 border-b border-border">
                  {['Operación', 'Estado', 'Progreso', 'Partes', 'Firma tentativa', 'Riesgo', ''].map(h => (
                    <th key={h} className="px-4 py-3 text-left text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">
                      <span className="inline-flex items-center gap-1">{h}{h && h !== '' && <ArrowUpDown className="w-3 h-3 opacity-40" />}</span>
                    </th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {lista.map(op => {
                  ...
                  return (
                    <tr key={op.id} className="border-b border-border last:border-0 hover:bg-muted/40 transition-colors">
                      <td className="px-4 py-3.5">
                        <Link to={`/operaciones/${op.id}`} className="block">
                      ...
                      <td className="px-4 py-3.5"><StatusBadge variant={e.color}>{e.label}</StatusBadge></td>
                      <td className="px-4 py-3.5 w-[180px]">
                        <div className="flex items-center gap-2">
                          <div className="flex-1 h-1.5 rounded-full bg-muted overflow-hidden">
                            <div
                              className={`h-full rounded-full ${op.estado === 'cerrado' ? 'bg-success' : op.estado === 'observado' ? 'bg-destructive' : 'bg-primary'}`}
                              style={{ width: `${op.progreso}%` }}
                            />
                          </div>
                          <span className="text-[11px] font-semibold text-muted-foreground num-tabular w-8 text-right">{op.progreso}%</span>
                        </div>
                      </td>
                      <td className="px-4 py-3.5">
                      ...
                      <td className="px-4 py-3.5">
                        <div className="flex items-center gap-2">
                          ...
                          <span className={`text-[11px] ${op.diasFirma < 7 && op.diasFirma > 0 ? 'text-destructive font-semibold' : 'text-muted-foreground'}`}>
                            {op.diasFirma > 0 ? `en ${op.diasFirma}d` : op.diasFirma === 0 ? 'hoy' : `hace ${-op.diasFirma}d`}
                          </span>
                        </div>
                      </td>
                      <td className="px-4 py-3.5"><StatusBadge variant={r.color}>{r.label}</StatusBadge></td>
                      <td className="px-4 py-3.5">
                        <Link to={`/operaciones/${op.id}`} className="inline-flex items-center justify-center w-8 h-8 rounded-lg hover:bg-muted text-muted-foreground hover:text-foreground transition-colors">
          ...
          <div className="px-4 py-3 flex items-center justify-between text-[12px] text-muted-foreground border-t border-border bg-muted/30">
            <div>Mostrando <span className="font-semibold text-foreground">{lista.length}</span> de <span className="font-semibold text-foreground">{operaciones.length}</span> legajos</div>
